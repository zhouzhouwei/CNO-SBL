function [x, iter,L1] = Ga_FSBL(y, Phi, paras, InitVal)
% Gamma hyperprior
...
L1 = obj_SBL(y, Phi, theta, gamma, lambda, paras);
...
% objective function
function L = obj_SBL(y,Phi,x,gamma,lambda,paras)
thre = 1e-8 ;
b0 = paras.b0 ;
c1 = 2*paras.c0 - 2 ;
d0 = paras.d0 ;
...
n1 = n+2-m-2*a0 ;
R = norm(y-Phi*x)^2 ;
L = sum(log(lambda+s0*gamma+thre) + c1*log(gamma+thre) + (2*d0 + x.^2)./(gamma+thre)) ....
    +(R+2*b0)./(lambda+thre) - n1*log(lambda+thre) ;
end